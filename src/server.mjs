...
const SESSION_STORAGE_DIR = path.resolve(process.env.SESSION_STORAGE_DIR || "/data/baileys-auth");
...
function aggregateStatus() {
  ...
  const selected = priority.map((status) => list.find((item) => item.status === status)).find(Boolean) || list[0] || {
    id: null,
    status: "disconnected",
...
async function sendWebhook(sessionId, event, data = {}) {
  ...
  try {
    ...
    log("error", "falha ao enviar webhook", { sessionId, event, error: String(error) });
  }
}
...
async function connectSession(sessionId, { forceFresh = false } = {}) {
  ...
  try {
    ...
    const socket = makeWASocket({
      auth: state,
      version,
      browser: Browsers.ubuntu("Impulsão AI"),
    ...
    socket.ev.on("connection.update", async (update) => {
      ...
      if (connection === "connecting") {
        await updateSessionState(sessionId, { status: "connecting", lastError: null });
      ...
      if (connection === "open") {
        ...
        log("info", "sessão restaurada com sucesso", { sessionId, restored: true });
      ...
      if (connection === "close") {
        ...
        log(loggedOut ? "warn" : "error", loggedOut ? "sessão perdida / logged out" : "conexão fechada", {
          sessionId,
          reason,
        });
        ...
        current.reconnectTimer = setTimeout(() => {
          connectSession(sessionId).catch((error) => {
            log("error", "erro ao reconectar sessão", { sessionId, error: String(error) });
          });
        }, delay);
      }
    });
    ...
    socket.ev.on("messages.upsert", async ({ messages = [], type }) => {
      ...
      for (const msg of messages) {
        ...
        log("info", "mensagem recebida", { sessionId, from, messageId: payload.id });
...
app.get("/qr", async (req, res) => {
  ...
  if (!current.socket && current.status !== "connected") {
    current = await connectSession(sessionId, { forceFresh: current.status === "logged_out" });
  }
...
app.get("/session/create", async (req, res) => {
  ...
  current = getSessionState(sessionId);
  return res.json({ ok: true, session: publicSession(current), qr: current.qr });
});
...
app.get("/session/:sessionId/delete", async (req, res) => {
  ...
  return res.json({ ok: true, session: publicSession(getSessionState(sessionId)) });
});
...
app.post("/session/:sessionId/send", async (req, res) => {
  ...
  if (!sessionId) return res.status(400).json({ ok: false, error: "sessionId required" });
  if (!to || to.length < 10 || to.length > 15) return res.status(400).json({ ok: false, error: "invalid destination" });
  if (!message) return res.status(400).json({ ok: false, error: "message required" });

  const current = await ensureSession(sessionId);
  if (!current.socket || current.status !== "connected") {
    return res.status(409).json({ ok: false, error: "session not connected", status: current.status });
  }
  ...
  return res.json({ ok: true, messageId, session: publicSession(current) });
});
...
async function bootstrap() {
  ...
  for (const sessionId of sessionDirs) {
    ...
    connectSession(sessionId).catch((error) => {
      log("error", "falha ao restaurar sessão", { sessionId, error: String(error) });
    });
  }
}
...
bootstrap()
  ...
  .catch((error) => {
    log("error", "falha crítica ao iniciar worker", { error: String(error) });
    process.exit(1);
  });
